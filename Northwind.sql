...
#List all the products with their corresponding category:
Select c.categoryID,c.categoryName,p.productID,p.ProductName from categories c join products p on c.categoryID=p.categoryID;

#Find orders made by a particular customer:
select* from orders where customerID=34;

#Get the total number of orders handled by each employee:
select e.employeeID,e.firstname,e.lastname, count(o.orderID) as totalorders from employees e
left join orders o on e.employeeID=o.employeeID group by e.employeeID,e.firstname,e.lastname;

#Find the top-selling products (by quantity) along with their suppliers:

select p.productname,s.suppliername, max(od.quantity) as topquantity 
from products p 
inner join orderdetails od on p.productID= od.productID
inner join suppliers s on p.SupplierID=s.SupplierID 
group by p.productname,s.suppliername order by topquantity desc;
...
select o.orderid,e.firstname,e.lastname,c.customername from orders o 
inner join employees e on o.employeeid=e.employeeid
inner join customers c on o.customerid=c.customerid;

#Find out the number of products supplied by each supplier:
select s.supplierId,s.suppliername, count(p.productid) as noofproduct from suppliers s
left join products p on p.supplierid=s.supplierid
group by s.supplierId,s.suppliername;

#Find orders placed on a specific date:
select * from orders where orderdate between "1996-07-10" and "1997-10-19";

#List all employees born after a certain date:
select* from employees where birthdate between "1950-10-10" and "1997-12-12";

#Retrieve the details of a particular order along with the products ordered:
select* from products;
select o.orderdetailid,o.orderid,o.productID,o.quantity,p.productID,p.productname 
from orderdetails o join products p on o.productId=p.productID where orderID=10317;

#list all customers who have not placed orders
...
select od.orderID, count(p.productID) from products p
join orderdetails od on p.productID=od.ProductID
group by od.orderID;

#Retrieve the contact information of suppliers located in a specific country
select contactName, Phone from suppliers where country="japan";

#Calculate the total revenue generated by each product
select* from products;
select* from orderdetails;
select p.productID, p.productName, sum(p.price*od.quantity) as revenue from orderdetails od
join products p on od.productID=p.productID
group by P.productID, P.productName;

#Find the employee who handled the most orders
select* from employees;
select e.firstname, count(o.orderID) as maxorder from orders o 
join employees e on e.employeeId=o.employeeID
group by  e.firstname order by maxorder desc ;

#Retrieve orders placed by customers from a specific country, along with the corresponding shipping details
select o.customerID, o.orderID, c.customerName, s.shipperName from orders o 
...
group by country order by totalcustomers desc; 
